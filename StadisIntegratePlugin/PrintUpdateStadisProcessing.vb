...
'----------------------------------------------------------------------------------------------
<GuidAttribute(Discover.CLASS_PrintUpdateStadisProcessing)>
...
#Region " Receipt Processing "
    ...
    Private Function PostGCPurchase(ByVal invoiceHandle As Integer, ByVal itemHandle As Integer, ByVal tenderHandle As Integer) As Guid
        ...
        End With
        Dim sy As StadisIntegratePlugin.WebReference.StadisReply = Common.StadisAPI.GCPurchase(sr, LoadGiftCardsForWSCall(invoiceHandle, itemHandle), Common.LoadHeader(fAdapter, "Receipt", invoiceHandle), Common.LoadItems(fAdapter, "Receipt", invoiceHandle, itemHandle), Common.LoadTenders(fAdapter, "Receipt", invoiceHandle, tenderHandle))
    ...
    Private Function LoadGiftCardsForWSCall(ByVal invoiceHandle As Integer, ByVal itemHandle As Integer) As StadisIntegratePlugin.WebReference.StadisGiftCard()
        Dim mStadisGiftCards(mGiftCardCount - 1) As StadisIntegratePlugin.WebReference.StadisGiftCard
        ...
        While Not fAdapter.EOF(itemHandle)
            ...
            If Stadis(0) = "STADIS" Then
                ...
                If itemType = "I" OrElse itemType = "A" Then
                    Dim mStadisGiftCard As New StadisIntegratePlugin.WebReference.StadisGiftCard
    ...
    Private Function PostTransaction(ByVal invoiceType As String, ByVal invoiceHandle As Integer, ByVal itemHandle As Integer, ByVal tenderHandle As Integer) As Guid
        Dim oRequest As New StadisIntegratePlugin.WebReference.StadisRequest
        With oRequest
            .ReferenceNumber = Common.BOGetStrAttributeValueByName(fAdapter, invoiceHandle, "Invoice Number")
            .RegisterID = Common.BOGetStrAttributeValueByName(fAdapter, invoiceHandle, "Invoice Workstion")
            .ReceiptID = Common.BOGetStrAttributeValueByName(fAdapter, invoiceHandle, "Invoice Number")
            ...
            .TenderID = ""
        End With
        Dim oReply As StadisIntegratePlugin.WebReference.StadisReply = Common.StadisAPI.SVPostTransaction(oRequest, Common.LoadHeader(fAdapter, "Receipt", invoiceHandle), Common.LoadItems(fAdapter, "Receipt", invoiceHandle, itemHandle), Common.LoadTenders(fAdapter, "Receipt", invoiceHandle, tenderHandle))
